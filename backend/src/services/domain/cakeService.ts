...
export class CakeService {
    ...
    public async findMany (): Promise<Cake[]> {
        return await this.cakeRepo.find();
    }
    ...
    public async createOne (data: {
        ...
        try {
            return await AppDataSource.transaction(async (manager) => {
                const cake = await this.cakeRepo.save({
                  ...
                  if (data.imagePaths.length) {
                    await this.cakeImageRepo.insert(data.imagePaths.map(path => {
                        return {
                            path,
                            cakeId: cake.id,
                        }
             ...
             */
            if (data.imagePaths.length) {
                await Promise.all(data.imagePaths.map(async (path) => {
                    await unlink(path);
                }));
            }
            throw error;
